...
func is_winning_cell(row: int, col: int) -> bool:
	for cell in winning_cells:
		if cell is Vector2i and cell.x == row and cell.y == col:
			return true
	return false
...
func _draw() -> void:
	...
	for r in range(ROWS):
		for c in range(COLS):
			...
			draw_rect(
				Rect2(cell_top_left, Vector2(cell_size, cell_size)),
			...
			if is_winning_cell(r, c):
				draw_arc(center, radius + 6.0, 0.0, TAU, 64, Color(0.2, 1.0, 0.2), 4.0)

			# Highlight last move
			elif r == last_move_row and c == last_move_col and last_move_row != -1:
